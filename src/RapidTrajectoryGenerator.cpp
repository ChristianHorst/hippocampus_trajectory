...
RapidTrajectoryGenerator::RapidTrajectoryGenerator(const Vec3 x0, const Vec3 v0, const Vec3 a0)
{
...
RapidTrajectoryGenerator::InputFeasibilityResult RapidTrajectoryGenerator::CheckInputFeasibilitySection(double fminAllowed, double fmaxAllowed, double wmaxAllowed, double t1, double t2, double minTimeSection)
{
  ...
  {
    double amin, amax;
    _axis[i].GetMinMaxAcc(amin, amax, t1, t2); //to do fmax= ma+cv

    //distance from zero thrust point in this axis
    double v1 = 2.6 * amax + 5.4 * 0.5 ; //left GetAcceleration(t).Multi(2.6) +  GetVelocity(t).Multi(5.4) 
    double v2 = 2.6 * amax + 5.4 * 0.5 ;; //right
