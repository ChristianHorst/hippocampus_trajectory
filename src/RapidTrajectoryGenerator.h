...
public:
  ...
  Vec3   GetNormalVector(double t) const {return (Vec3(0,0,0.0000001)+GetAcceleration(t).Multi(2.6) +  GetVelocity(t).Multi(5.4) ).GetUnitVector();};
  //! Return the quadrocopter's thrust input along the trajectory at time t
  double GetThrust(double t) const     {return ( GetAcceleration(t).Multi(2.6) +  GetVelocity(t).Multi(5.4) ).GetNorm2();};
