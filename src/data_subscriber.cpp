...
data_subscriber::data_subscriber(ros::NodeHandle* nodehandle):nh(*nodehandle)
{
 ...
 /*   void data_subscriber::CurrentAxisCallback(const mavros_msgs::HippocampusControl::ConstPtr &axis){
        ROS_INFO("in callback function currentaxis i heard %f:", axis->roll_effort);
        data_subscriber::current_axis = Vec3(axis->roll_effort, axis->pitch_effort, axis->yaw_effort);
    }
    */
